...
echo ""
echo "Step 4: Writing environment variables to $BASHRC..."

# Remove existing block if present (idempotency)
if grep -q "$GUARD_START" "$BASHRC" 2>/dev/null; then
  TMPFILE=$(mktemp)
  awk "/$GUARD_START/{found=1} !found{print} /$GUARD_END/{found=0}" "$BASHRC" > "$TMPFILE"
  mv "$TMPFILE" "$BASHRC"
  echo "  ✓ Removed previous agent-infra block"
fi
